...
var BASE_URL = 'http://52.14.196.78:3232/bws/api';
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  console.log('api-network:'+ opts.network);
...
API._buildSecret = function(walletId, walletPrivKey,network) {
  ...
  return _.padRight(widBase58, 22, '0') + walletPrivKey.toWIF() + (network == 'testnet' ? 'T' : 'L') + 'dash';
};
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  self._doGetRequest('/v2/feelevels/?coin=' + '&network=' + (network || 'livenet'), function(err, result) {
    if (err) return cb(err);
...
var coin = 'dash';
  if (!_.includes(['dash'], coin)) return cb(new Error('Invalid coin'));
  console.log('gl--opts2:'+ JSON.stringify(opts));
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {  
  if (!_.includes(['dash'], opts.coin)) return cb(new Error('Invalid coin'));
