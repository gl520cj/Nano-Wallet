...
angular.module('copayApp.controllers').controller('confirmController', function($rootScope, $scope, $interval, $filter, $timeout, $ionicScrollDelegate, gettextCatalog, walletService, platformInfo, lodash, configService, $stateParams, $window, $state, $log, profileService, bitcore, bitcoreCash, bitcoreDash,bitcoreNano,txFormatService, ongoingProcess, $ionicModal, popupService, $ionicHistory, $ionicConfig, payproService, feeService, bwcError, txConfirmNotification, externalLinkService) {
  ...
  $scope.$on("$ionicView.beforeEnter", function(event, data) {
    ...
    // Setup $scope
    var B = data.stateParams.coin == 'bch' ? bitcoreCash : data.stateParams.coin == 'dash' ? bitcoreDash : data.stateParams.coin == 'Nano' ? bitcoreNano : bitcore;
    var networkName;
    try {
      networkName = (new B.Address(data.stateParams.toAddress)).network.name;
    } catch (e) {
      var message = gettextCatalog.getString('Copay only supports Bitcoin  using new version numbers addresses');
