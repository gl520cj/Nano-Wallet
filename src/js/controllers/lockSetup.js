...
angular.module('copayApp.controllers').controller('lockSetupController', function($state, $rootScope, $scope, $timeout, $log, configService, gettextCatalog, fingerprintService, profileService, lodash, applicationService) {
  ...
  function init() {
    ...
    //   $scope.options.push({
    //     method: 'fingerprint or faceid',
    //     label: gettextCatalog.getString('Lock by Fingerprint or Faceid'),
    //     needsBackup: false,
    //     disabled: false,
    //   });
    // }
  ...
  function initMethodSelector() {
    ...
    if (fingerprintService.isAvailable()) {
      switch (savedMethod) {
        case 'pin':
          disable('fingerprint or faceid');
          break;
        case 'fingerprint or faceid':
          disable('pin');
          break;
      }
    }
  ...
  function processWallets() {
    ...
    if (singleLivenetWallet) {
      ...
      $scope.errorMsg = gettextCatalog.getString('Backup all main wallets before using this function');
      disableOptsUntilBackup();
    } else {
  ...
  function enableMethod(method) {
    switch (method) {
      case 'pin':
        ...
        break;
      case 'fingerprint or faceid':
        saveConfig('fingerprint or faceid');
        break;
    }
  };
